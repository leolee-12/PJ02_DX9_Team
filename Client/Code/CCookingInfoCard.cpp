...
HRESULT CCookingInfoCard::Ready_GameObject()
{
	...
	pGameObject = CCookingTargetFood::Create(m_pGraphicDev, { 320.0f,150.0f,0.001f }, 0.3f, FOODTYPE::FT_NOMAL);
	if (nullptr == pGameObject)
		return E_FAIL;
	m_vecCookingSelectUI.push_back(pGameObject);

	pGameObject = CCookingSelectSlot::Create(m_pGraphicDev, { 320.0f,150.0f,0.01f }, 0.3f);
	if (nullptr == pGameObject)
		return E_FAIL;
	m_vecCookingSelectUI.push_back(pGameObject);

	pGameObject = CCookingInfoRecipe::Create(m_pGraphicDev, { 300,-140,0.001f }, 0.3f);
	if (nullptr == pGameObject)
		return E_FAIL;
	m_vecCookingSelectUI.push_back(pGameObject);

	pGameObject = CCookingInfoFaith::Create(m_pGraphicDev, { 200,-200,0.001f }, 0.3f);
	if (nullptr == pGameObject)
		return E_FAIL;
	m_vecCookingSelectUI.push_back(pGameObject);

	_vec3 DividerPos = _vec3(410.f, 140.f, 0.1f);
	pGameObject = CDivider::Create(m_pGraphicDev, 1, DividerPos, 0.01f);
	if (nullptr == pGameObject)
		return E_FAIL;
	m_vecCookingSelectUI.push_back(pGameObject);

	_vec3 DividerPos1 = _vec3(230.f, 140.f, 0.1f);
	pGameObject = CDivider::Create(m_pGraphicDev, 0, DividerPos1, 0.01f);
	if (nullptr == pGameObject)
		return E_FAIL;
	m_vecCookingSelectUI.push_back(pGameObject);

	pGameObject = CCookingUpDownArrow::Create(m_pGraphicDev, 0, { 230.0f,-200.0f,0.001f }, 0.9f);
	if (nullptr == pGameObject)
		return E_FAIL;
	m_vecCookingSelectUI.push_back(pGameObject);

	//pGameObject = CCookingUpDownArrow::Create(m_pGraphicDev, 1, { 170,-50,0.001f }, 0.9f);
	//if (nullptr == pGameObject)
	//	return E_FAIL;
	//m_vecCookingSelectUI.push_back(pGameObject);

	m_pDescBg = CInvenSlot::Create(m_pGraphicDev, _vec3{ 0.0f,-50.0f,0.01f }, _vec3{320.0f,0.0f,0.0f}, 1.5f, 1.0f);
	if (nullptr == m_pDescBg)
		return E_FAIL;
	...
	m_pResourceFont->Set_Pos(_vec2(360.0f, -140.0f));
	m_pResourceFont->Set_Scale(_vec2(59.f * 2.f, 123.f * 0.5f));
	m_pResourceFont->Set_Font(L"Font_Default");
	...
	m_pRecipeFont->Set_Pos(_vec2(330.0f, -140.0f));
	m_pRecipeFont->Set_Scale(_vec2(59.f * 2.f, 123.f * 0.5f));
	m_pRecipeFont->Set_Font(L"Font_Default24");
	m_pRecipeFont->Active();


	for (int i = 0; i < 3; ++i)
	{
		pGameObject = CCookingStar::Create(m_pGraphicDev, 1, { 290.0f + (i * 30.0f),50.0f,0.001f }, 0.2f);
...
_int CCookingInfoCard::Update_GameObject(const _float& fTimeDelta)
{
	...
	m_pDescBg->Update_GameObject(fTimeDelta);
...
void CCookingInfoCard::LateUpdate_GameObject(const _float& fTimeDelta)
{
	...
	m_pDescBg->LateUpdate_GameObject(fTimeDelta);
...
void CCookingInfoCard::Render_GameObject()
{
	...
	RECT rc0Player = { 870, 250, 1050, 300 };
	CFontMgr::GetInstance()->Render_Font(L"Font_Default30_Heavy", szFoodName, rc0Player, FontColor, DT_CENTER | DT_TOP);

	swprintf_s(szFoodName, L"정말맛있다");
	RECT rc1Player = { 870, 350, 1050, 400 };
	CFontMgr::GetInstance()->Render_Font(L"Font_Default24", szFoodName, rc1Player, FontColor, DT_CENTER | DT_TOP);

	swprintf_s(szFoodName, L"간장밥입니다 계란을곁들인");
	RECT rc2Player = { 700, 400, 1200, 450 };
	CFontMgr::GetInstance()->Render_Font(L"Font_Default24", szFoodName, rc2Player, FontColor, DT_CENTER | DT_TOP);

	FontColor = D3DXCOLOR(255.f / 256.f, 0.f / 256.f, 0.f / 256.f, 1.f);
	swprintf_s(szFoodName, L"재료");
	RECT rc3Player = { 870, 450, 1050,  470 };
	CFontMgr::GetInstance()->Render_Font(L"Font_Default24", szFoodName, rc3Player, FontColor, DT_CENTER | DT_TOP);
