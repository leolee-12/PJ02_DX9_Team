...
CLoadingCircle::CLoadingCircle(LPDIRECT3DDEVICE9 pGraphicDev)
	: CUi(pGraphicDev), m_pBufferCom(nullptr), m_pTransformCom(nullptr), m_bRander(true)
{
	ZeroMemory(&m_vPos, sizeof(_vec3));
}

CLoadingCircle::CLoadingCircle(LPDIRECT3DDEVICE9 pGraphicDev, IMessageChannel* pMessageChannel)
	: CUi(pGraphicDev, pMessageChannel), m_pBufferCom(nullptr), m_pTransformCom(nullptr), m_bRander(true)
{
	ZeroMemory(&m_vPos, sizeof(_vec3));
}
...
_int CLoadingCircle::Update_GameObject(const _float& fTimeDelta)
{
	...
	if (m_bRander) { CRenderer::GetInstance()->Add_RenderGroup(RENDER_UI, this); }
