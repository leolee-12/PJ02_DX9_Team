...
void CLoading::Render_Scene()
{
	...
	if (true == m_pLoading->Get_Finish()) {
		CFontMgr::GetInstance()->Render_Font(L"Font_Lapture40", L"�ε� �Ϸ�!", &vPos, D3DXCOLOR(0.75f, 0.75f, 0.75f, 1.f), DT_NOCLIP);
	}
	else {
		CFontMgr::GetInstance()->Render_Font(L"Font_Lapture40", L"�ε���.....", &vPos, D3DXCOLOR(0.75f, 0.75f, 0.75f, 1.f), DT_NOCLIP);
	}
